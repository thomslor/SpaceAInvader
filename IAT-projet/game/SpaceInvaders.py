...
class SpaceInvaders():
    ...
    def get_state(self):
        ...
        """
        posYAlien = self.invader_Y[0]
        count = 0
        id = 0
        for pos in self.invader_Y:
            if posYAlien == pos:
                count += 1
                pass
            else:
                if pos < posYAlien:
                    ...
                    id = count
                    count += 1
        posXAlien = self.invader_X[id]
        bullet_state = 0

        if self.get_bullet_state is "rest":
            bullet_state = 0
        ...
        return np.array([posXPlayer, posXAlien, posYAlien, bullet_state])
